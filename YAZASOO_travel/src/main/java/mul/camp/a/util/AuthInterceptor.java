 ...
 * javax.servlet.http.HttpServletResponse; import javax.websocket.Session;
 * 
 ...
 * 
 * try { sessionVo = (Session) SessionUtils.getSessionAttribute(request,
 * "sessUser");
 * 
 * if (sessionVo != null && sessionVo.getSessUserID() != null) { return true; }
 * else { ModelAndView modelAndView = new ModelAndView("redirect:/forward.do");
 * modelAndView.addObject("msgCode", "세션이 만료되어 로그아웃 되었습니다. 다시 로그인 해주세요.");
 * modelAndView.addObject("returnUrl", "/login.do"); throw new
 * ModelAndViewDefiningException(modelAndView); } } catch (Exception e) {
